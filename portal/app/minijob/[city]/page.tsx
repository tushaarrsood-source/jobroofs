...
export async function generateMetadata({
  ...
  return getProgrammaticMetadata('minijob', city);
}

export default async function MinijobCityPage({
  params,
}: {
  params: Promise<{ city: string }>;
}) {
  ...
  const jobs = await getProgrammaticJobs('minijob', cityDef.id);

  return (
    <ProgrammaticLandingPage
      city={cityDef}
      jobType={PROGRAMMATIC_JOB_TYPES.minijob}
      jobs={jobs}
    />
  );
}
