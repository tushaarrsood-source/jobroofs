...
export async function generateMetadata({
  ...
  return getProgrammaticMetadata('werkstudent', city);
}

export default async function WerkstudentCityPage({
  params,
}: {
  params: Promise<{ city: string }>;
}) {
  ...
  const jobs = await getProgrammaticJobs('werkstudent', cityDef.id);

  return (
    <ProgrammaticLandingPage
      city={cityDef}
      jobType={PROGRAMMATIC_JOB_TYPES.werkstudent}
      jobs={jobs}
    />
  );
}
