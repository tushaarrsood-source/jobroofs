...
export async function generateMetadata({
  ...
  return getProgrammaticMetadata('teilzeit', city);
}

export default async function TeilzeitCityPage({
  params,
}: {
  params: Promise<{ city: string }>;
}) {
  ...
  const jobs = await getProgrammaticJobs('teilzeit', cityDef.id);

  return (
    <ProgrammaticLandingPage
      city={cityDef}
      jobType={PROGRAMMATIC_JOB_TYPES.teilzeit}
      jobs={jobs}
    />
  );
}
