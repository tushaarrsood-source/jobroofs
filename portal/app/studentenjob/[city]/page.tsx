...
export async function generateMetadata({
  ...
  return getProgrammaticMetadata('studentenjob', city);
}

export default async function StudentenjobCityPage({
  params,
}: {
  params: Promise<{ city: string }>;
}) {
  ...
  const jobs = await getProgrammaticJobs('studentenjob', cityDef.id);

  return (
    <ProgrammaticLandingPage
      city={cityDef}
      jobType={PROGRAMMATIC_JOB_TYPES.studentenjob}
      jobs={jobs}
    />
  );
}
