...
export function AiJobCreatorChat({
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="rounded-2xl border bg-white p-5 sm:p-6 shadow-xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-auto">
          <button
            type="button"
            onClick={handleReset}
            className="apple-press inline-flex items-center gap-2 px-3.5 py-2 rounded-xl border bg-white text-sm font-medium text-zinc-700 hover:text-black hover:bg-zinc-50 transition cursor-pointer"
      ...
      {/* Conditional rendering: Chat view vs Preview view */}
      {showPreview ? (
        // ---- Preview Card ----
        <div className="rounded-2xl border bg-white p-5 sm:p-6 shadow-xs">
          ...
          <div className="mt-3.5 w-full h-2 bg-zinc-100 rounded-full overflow-hidden">
            <div className="h-full bg-emerald-600 transition-all duration-500 rounded-full" style={{ width: `${completeness}%` }} />
          </div>
          <div className="mt-5 rounded-2xl bg-[#fafaf9] p-5 space-y-4 shadow-2xs">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-lg bg-black text-white px-2.5 py-1 text-xs font-mono font-bold uppercase tracking-wider">Job</span>
              <span className="rounded-lg bg-white border px-2.5 py-1 text-xs font-mono font-semibold text-zinc-800">
                {extractedJob.employmentType || (isDe ? 'Minijob (bis 603 €)' : 'Minijob / Part-Time')}
              </span>
              {extractedJob.wage && (
                <span className="rounded-lg bg-emerald-100 text-emerald-900 border px-2.5 py-1 text-xs font-mono font-bold">
                  {extractedJob.wage}
                </span>
              )}
            </div>
            <div>
              ...
              <div className="mt-1.5 flex items-center gap-2.5 text-sm sm:text-base text-zinc-700 font-medium">
            ...
            <div className="text-[14.5px] sm:text-[15px] text-zinc-800 bg-white p-4 sm:p-5 rounded-xl border text-sm leading-relaxed min-h-[90px] shadow-2xs">
            ...
            <div className="pt-1 flex flex-wrap gap-2 text-xs sm:text-sm font-medium text-zinc-800">
              ...
              {!extractedJob.whatsapp && !extractedJob.contactEmail && !extractedJob.phone && (
                <span className="text-zinc-500 italic">{isDe ? 'Noch kein Direktkontakt (WhatsApp oder E-Mail) hinterlegt' : 'No direct contact (WhatsApp or Email) added yet'}</span>
              )}
            </div>
            {/* Action buttons */}
            {isReady ? (
              <div className="space-y-3 mt-5">
                <button
                  ...
                  className="apple-press w-full py-3.5 px-5 rounded-2xl bg-black hover:bg-zinc-800 text-white text-base font-bold transition cursor-pointer flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="apple-press w-full py-2.5 px-4 rounded-xl border bg-white hover:bg-zinc-50 text-sm font-semibold text-zinc-800"
                >
              ...
              <div className="space-y-2.5 mt-5">
                <button
                  ...
                  onClick={() => onApplyToForm(extractedJob, 1)}
                  className="apple-press w-full py-3 px-4 rounded-xl border bg-white hover:bg-zinc-50 disabled:opacity-40 text-sm font-semibold text-black flex items-center justify-center gap-2"
                >
                ...
                <p className="text-xs sm:text-sm text-zinc-500 text-center font-normal">
                  {isDe ? 'Nenne einfach noch kurz deinen Betrieb und deine WhatsApp-Nummer im Dialog.' : 'Simply mention your business name and WhatsApp number in the chat.'}
                </p>
              </div>
            )}
          </div>
          <button
            ...
            className="mt-4 apple-press inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-100 hover:bg-zinc-200 text-sm font-medium text-black"
          >
            {isDe ? 'Zurück zum Chat' : 'Back to Chat'}
          </button>
        </div>
      ) : (
        // ---- Chat Pane ----
        <div className="flex flex-col rounded-2xl border bg-white shadow-xs overflow-hidden h-[620px] sm:h-[660px]">
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`flex items-start gap-3 sm:gap-3.5 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}
              >
                <div
                  className={`size-9 rounded-2xl flex items-center justify-center shrink-0 text-sm font-bold ${msg.role === 'user' ? 'bg-zinc-800 text-white' : 'bg-black text-white shadow-xs'}`}
                >
                  {msg.role === 'user' ? <User className="size-4.5" /> : <span className="font-bold text-xs tracking-wider">JR</span>}
                </div>
                <div
                  className={`max-w-[85%] sm:max-w-[82%] rounded-2xl px-4.5 py-3.5 sm:px-5 sm:py-4 text-[15px] sm:text-base leading-relaxed ${msg.role === 'user' ? 'bg-black text-white rounded-tr-xs' : 'bg-[#f7f7f6] text-zinc-900 rounded-tl-xs border border-zinc-200/80'}`}
                >
                  <p className="whitespace-pre-wrap">{msg.content}</p>
                  <div className={`mt-1.5 text-xs ${msg.role === 'user' ? 'text-zinc-400 text-right' : 'text-zinc-500'}`}>{msg.timestamp}</div>
                </div>
              </div>
            ))}
            {loading && (
          ...
          {quickReplies.length > 0 && !loading && (
            <div className="px-4 sm:px-6 py-2.5 border-t bg-[#fafaf9] flex flex-wrap gap-2">
              {quickReplies.map((qr, i) => (
                <button
                  key={i}
                  type="button"
                  onClick={() => handleSendMessage(qr)}
                  className="apple-press text-xs sm:text-sm font-medium bg-white hover:bg-zinc-100 hover:border-black text-zinc-800 border border-zinc-200 rounded-full px-3.5 sm:px-4 py-1.5 transition-all"
                >
                  {qr}
                </button>
              ))}
          ...
          <div className="p-3 sm:p-4 border-t bg-white">
            ...
            >
              <textarea
                ...
                placeholder={isDe ? 'Beschreibe die Stelle oder antworte hier... (Enter zum Senden)' : 'Describe the job or reply here... (Enter to send)'}
                rows={1}
                className="flex-1 max-h-28 min-h-[50px] sm:min-h-[54px] px-4 py-3 text-base text-black placeholder:text-zinc-400 border border-zinc-300 rounded-2xl bg-[#fafaf9] focus:bg-white focus:border-black focus:ring-1 focus:ring-black outline-none resize-none transition-colors"
              />
              <button
                type="submit"
                disabled={!input.trim() || loading}
                className="apple-press size-12 sm:size-13 rounded-2xl bg-black hover:bg-zinc-800 text-white flex items-center justify-center shrink-0 disabled:opacity-40 transition"
                title={isDe ? 'Nachricht senden' : 'Send message'}
              >
                <Send className="size-5" />
              </button>
            </form>
            <div className="mt-2 px-1 flex items-center justify-between text-xs text-zinc-500 font-normal">
              <span>{isDe ? 'Du kannst auch ganze Stellenbeschriften oder WhatsApp-Notizen einfügen.' : 'You can paste full job drafts or WhatsApp notes.'}</span>
              <span className="hidden sm:inline font-mono text-[11px] text-zinc-400">Shift+Enter = {isDe ? 'Zeilenumbruch' : 'New line'}</span>
            </div>
          </div>
          {/* Show Preview button – appears when job is ready and preview not shown */}
          {isReady && !showPreview && (
            <div className="p-4 border-t bg-white flex justify-center">
              <button
                type="button"
                onClick={() => setShowPreview(true)}
                className="apple-press px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-medium transition"
              >
